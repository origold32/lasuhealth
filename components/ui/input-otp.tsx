...
const InputOTPGroup = React.forwardRef<React.ElementRef<"div">, React.ComponentPropsWithoutRef<"div">>(({ className, ...props }, ref) => (
  <div ref={ref} className={cn("flex items-center w-full gap-4", className)} {...props} />
));
...
const InputOTPSlot = React.forwardRef<React.ElementRef<"div">, React.ComponentPropsWithoutRef<"div"> & { index: number }>(({ index, className, ...props }, ref) => {
  ...
  return (
    <div
      ...
      className={cn("relative flex h-12 flex-1 items-center justify-center rounded-lg border", isActive && "z-10 ring-2 ring-offset-2 ring-ring ring-offset-background !border-l-input", className)}
      {...props}
    >
      {char}
      {hasFakeCaret && (
